...
float Tr : CONTROLOBJECT < string name = "FireParticleSystem_SnowCrystal1000.x"; string item = "Tr"; >;
...
VS_OUTPUT FireParticleSystem_Vertex_Shader_main(float4 Pos: POSITION){
   ...
   float s1 = time_0_X + 3.14159265 * 2 * frac(Pos.z * 15);
   float s4 = abs(cos(s1));

   pos.x = particleSpread * s * cos(62 * Pos.z + 0.01 * sin(s1));
   pos.z = particleSpread * s * sin(1830 * Pos.z);
   // Particles goes up
...
//   return color *tex2D(Flame, float2(1+(texCoord.x-1)/200-texCoord.z, -texCoord.y*0.5+0.5));
   return Tr * Tr * color * tex2D(Flame, float2(1+(texCoord.x-1)/200-frac(texCoord.z*10), -texCoord.y*0.5+0.5));
